...
configCtrl.empresaConfig = (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "hubo un error al intentar Obtener los datos",
...
configCtrl.updateMensajesLey = async (req, res) => {
  ...
  try {
    const mensActualizado = await MensajesLey.findByIdAndUpdate(
      id,
      { $set: edicionMensajes },
      { new: true }
    );
...
configCtrl.saveNumActual = async (req, res) => {
  ...
  try {
    ...
    res
      ...
      .json({
        message:
          "Hubo un error al intentar guardar/actualizar el número actual",
        details: error.message,
      });
  }
};
...
configCtrl.updateNumActual = async (req, res) => {
  ...
  try {
    const numActualizado = await Config.findByIdAndUpdate(
      id,
      { $set: numEditado },
      { new: true }
    );
    if (!numActualizado) {
      return res
        .status(404)
        .json({ message: "no se encontro registro en numActual" });
    }
    ...
    res
      .status(500)
      .json({
        error: "Error al intentar actualizar el numero Actual",
        details: error.message,
      });
  }
};
